...
var Cmd = &cobra.Command{
	Use:     "managedDatabase ID",
	Aliases: []string{"managedDatabases"},
	Short:   "Get a managedDatabase by ID",
	Long:    "Get a managedDatabase by ID and display its details",
...
func run(cmd *cobra.Command, argv []string) error {
	...
	resp, err := conn.Get(urls.ManagedDatabasePath(id), nil)
	if err != nil {
		return fmt.Errorf("can't retrieve managedDatabase: %v", err)
	}
