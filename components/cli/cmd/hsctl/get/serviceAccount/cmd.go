...
var Cmd = &cobra.Command{
	...
	Short:   "Get an OpenShell gateway service account",
...
func run(cmd *cobra.Command, argv []string) error {
	...
	defer conn.Close()
	response, _, err := serviceaccount.Request(conn, http.MethodGet, serviceaccount.ItemPath(args.gatewayID, argv[0]), nil, nil, http.StatusOK)
	if err != nil {
