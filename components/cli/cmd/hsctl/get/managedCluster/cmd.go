...
var Cmd = &cobra.Command{
	Use:     "managedCluster ID",
	Aliases: []string{"managedClusters"},
	Short:   "Get a managedCluster by ID",
	Long:    "Get a managedCluster by ID and display its details",
...
func run(cmd *cobra.Command, argv []string) error {
	...
	resp, err := conn.Get(urls.ManagedClusterPath(id), nil)
	if err != nil {
		return fmt.Errorf("can't retrieve managedCluster: %v", err)
	}
