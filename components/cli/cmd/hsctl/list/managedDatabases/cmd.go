...
var Cmd = &cobra.Command{
	Use:     "managedDatabases [flags]",
	Aliases: []string{"managedDatabase"},
	Short:   "List managedDatabases",
	Long:    "List managedDatabases, optionally filtering by search query",
	Args:    cobra.NoArgs,
	RunE:    run,
}
...
func init() {
	...
	arguments.AddColumnsFlag(fs, &args.columns, "id, connection_secret, engine, engine_version, instance_class, created_at")
...
func run(cmd *cobra.Command, argv []string) error {
	...
	page := 1
	for {
		resp, err := conn.List(urls.ManagedDatabasesPath, page, size, searchQuery, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve managedDatabases: %v", err)
		}
...
func listJSON(conn *connection.Connection, search string) error {
	...
	for {
		resp, err := conn.List(urls.ManagedDatabasesPath, page, size, search, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve managedDatabases: %v", err)
		}
