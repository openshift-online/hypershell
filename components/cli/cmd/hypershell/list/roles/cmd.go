...
var Cmd = &cobra.Command{
	...
	Aliases: []string{"role"},
	Short:   "List roles",
	Long:    "List roles, optionally filtering by search query",
	Args:    cobra.NoArgs,
	RunE:    run,
}
...
func init() {
	...
	arguments.AddColumnsFlag(fs, &args.columns, "id, built_in, description, display_name, name, created_at")
...
func run(cmd *cobra.Command, argv []string) error {
	...
	page := 1
	for {
		resp, err := conn.List(urls.RolesPath, page, size, searchQuery, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve roles: %v", err)
		}
...
func listJSON(conn *connection.Connection, search string) error {
	...
	for {
		resp, err := conn.List(urls.RolesPath, page, size, search, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve roles: %v", err)
		}
