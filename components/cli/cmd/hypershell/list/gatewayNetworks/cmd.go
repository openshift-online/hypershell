...
var Cmd = &cobra.Command{
	Use:     "gatewayNetworks [flags]",
	Aliases: []string{"gatewayNetwork"},
	Short:   "List gatewayNetworks",
	Long:    "List gatewayNetworks, optionally filtering by search query",
	Args:    cobra.NoArgs,
	RunE:    run,
}
...
func init() {
	...
	arguments.AddColumnsFlag(fs, &args.columns, "id, fleet_id, hub_gateway_id, name, status, created_at")
...
func run(cmd *cobra.Command, argv []string) error {
	...
	page := 1
	for {
		resp, err := conn.List(urls.GatewayNetworksPath, page, size, searchQuery, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gatewayNetworks: %v", err)
		}
...
func listJSON(conn *connection.Connection, search string) error {
	...
	for {
		resp, err := conn.List(urls.GatewayNetworksPath, page, size, search, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gatewayNetworks: %v", err)
		}
