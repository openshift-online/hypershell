...
var Cmd = &cobra.Command{
	Use:     "gatewayReleases [flags]",
	Aliases: []string{"gatewayRelease"},
	Short:   "List gatewayReleases",
	Long:    "List gatewayReleases, optionally filtering by search query",
	Args:    cobra.NoArgs,
	RunE:    run,
}
...
func init() {
	...
	arguments.AddColumnsFlag(fs, &args.columns, "id, canary_duration, canary_percent, fleet_id, image, created_at")
...
func run(cmd *cobra.Command, argv []string) error {
	...
	page := 1
	for {
		resp, err := conn.List(urls.GatewayReleasesPath, page, size, searchQuery, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gatewayReleases: %v", err)
		}
...
func listJSON(conn *connection.Connection, search string) error {
	...
	for {
		resp, err := conn.List(urls.GatewayReleasesPath, page, size, search, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gatewayReleases: %v", err)
		}
