...
var Cmd = &cobra.Command{
	...
	Short:   "List gateways",
	Long:    "List gateways, optionally filtering by search query",
	Args:    cobra.NoArgs,
	RunE:    run,
}
...
func init() {
	...
	arguments.AddColumnsFlag(fs, &args.columns, "id, cluster_id, database_config, database_id, external_dns, created_at")
...
func run(cmd *cobra.Command, argv []string) error {
	...
	page := 1
	for {
		resp, err := conn.List(urls.GatewaysPath, page, size, searchQuery, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gateways: %v", err)
		}
...
func listJSON(conn *connection.Connection, search string) error {
	...
	for {
		resp, err := conn.List(urls.GatewaysPath, page, size, search, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gateways: %v", err)
		}
