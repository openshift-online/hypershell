...
var Cmd = &cobra.Command{
	...
	Short:   "List fleets",
	Long:    "List fleets, optionally filtering by search query",
	Args:    cobra.NoArgs,
	RunE:    run,
}
...
func init() {
	...
	arguments.AddColumnsFlag(fs, &args.columns, "id, description, name, status, created_at")
...
func run(cmd *cobra.Command, argv []string) error {
	...
	page := 1
	for {
		resp, err := conn.List(urls.FleetsPath, page, size, searchQuery, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve fleets: %v", err)
		}
...
func listJSON(conn *connection.Connection, search string) error {
	...
	for {
		resp, err := conn.List(urls.FleetsPath, page, size, search, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve fleets: %v", err)
		}
