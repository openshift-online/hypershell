...
var Cmd = &cobra.Command{
	Use:     "gatewayProfiles [flags]",
	Aliases: []string{"gatewayProfile"},
	Short:   "List gatewayProfiles",
	Long:    "List gatewayProfiles, optionally filtering by search query",
	Args:    cobra.NoArgs,
	RunE:    run,
}
...
func init() {
	...
	arguments.AddColumnsFlag(fs, &args.columns, "id, container_cpu_limit_max, container_cpu_request_default, container_memory_limit_max, container_memory_request_default, created_at")
...
func run(cmd *cobra.Command, argv []string) error {
	...
	page := 1
	for {
		resp, err := conn.List(urls.GatewayProfilesPath, page, size, searchQuery, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gatewayProfiles: %v", err)
		}
...
func listJSON(conn *connection.Connection, search string) error {
	...
	for {
		resp, err := conn.List(urls.GatewayProfilesPath, page, size, search, args.orderBy)
		if err != nil {
			return fmt.Errorf("can't retrieve gatewayProfiles: %v", err)
		}
