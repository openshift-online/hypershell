...
var Cmd = &cobra.Command{
	Use:   "gatewayProfile ID [flags]",
	Short: "Delete a gatewayProfile",
	Long: "Delete a gatewayProfile by ID.\n\n" +
		"Examples:\n" +
		"  hypershell delete gatewayProfile 2abc123\n" +
		"  hypershell delete gatewayProfile 2abc123 --yes",
...
func run(cmd *cobra.Command, argv []string) error {
	gatewayProfileID := argv[0]

	if !args.yes {
		fmt.Fprintf(os.Stderr, "Delete gatewayProfile %s? [y/N]: ", gatewayProfileID)
		var response string
		fmt.Scanln(&response)
	...
	path := urls.GatewayProfilesPath + "/" + gatewayProfileID
	resp, err := conn.Delete(path)
	if err != nil {
		return fmt.Errorf("can't delete gatewayProfile: %v", err)
	}
	...
	fmt.Fprintf(os.Stdout, "GatewayProfile %s deleted.\n", gatewayProfileID)
	return nil
}
