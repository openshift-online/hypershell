...
func run(cmd *cobra.Command, argv []string) error {
	managedClusterID := argv[0]

	if !args.yes {
		fmt.Fprintf(os.Stderr, "Delete managedCluster %s? [y/N]: ", managedClusterID)
		var response string
		fmt.Scanln(&response)
	...
	path := urls.ManagedClustersPath + "/" + managedClusterID
	resp, err := conn.Delete(path)
	if err != nil {
		return fmt.Errorf("can't delete managedCluster: %v", err)
	}
	...
	fmt.Fprintf(os.Stdout, "ManagedCluster %s deleted.\n", managedClusterID)
	return nil
}
