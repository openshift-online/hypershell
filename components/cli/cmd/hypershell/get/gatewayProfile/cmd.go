...
var Cmd = &cobra.Command{
	Use:     "gatewayProfile ID",
	Aliases: []string{"gatewayProfiles"},
	Short:   "Get a gatewayProfile by ID",
	Long:    "Get a gatewayProfile by ID and display its details",
...
func run(cmd *cobra.Command, argv []string) error {
	...
	resp, err := conn.Get(urls.GatewayProfilePath(id), nil)
	if err != nil {
		return fmt.Errorf("can't retrieve gatewayProfile: %v", err)
	}
