...
var Cmd = &cobra.Command{
	Use:     "gatewayRelease ID",
	Aliases: []string{"gatewayReleases"},
	Short:   "Get a gatewayRelease by ID",
	Long:    "Get a gatewayRelease by ID and display its details",
...
func run(cmd *cobra.Command, argv []string) error {
	...
	resp, err := conn.Get(urls.GatewayReleasePath(id), nil)
	if err != nil {
		return fmt.Errorf("can't retrieve gatewayRelease: %v", err)
	}
