...
func Location() (path string, err error) {
	if envConfig := os.Getenv("HYPERSHELL_CONFIG"); envConfig != "" {
	...
	path = filepath.Join(home, ".hypershell.json")
	...
	if os.IsNotExist(err) {
		...
		path = filepath.Join(configDir, "hypershell", "config.json")
	}
