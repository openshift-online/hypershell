...
describe("GET /api/metrics/cluster-memory", () => {
  ...
  it("returns cluster memory bytes when Prometheus succeeds", async () => {
    ...
    const response = await app.inject({
      method: "GET",
      url: "/api/metrics/cluster-memory",
    });
  ...
  it("allows dashboard administrators when OIDC is enabled", async () => {
    ...
    const prometheusUrl = await startPrometheusStub((request, response) => {
      ...
      if (query === clusterMemoryAvailablePromql) {
        response.end(prometheusSample("250"));
        return;
      }
      response.statusCode = 400;
      response.end();
