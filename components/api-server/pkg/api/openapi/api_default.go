...
type ApiGetGatewayDeletionStatusRequest struct {
	ctx               context.Context
	ApiService        *DefaultAPIService
	externalReference *string
}

func (r ApiGetGatewayDeletionStatusRequest) ExternalReference(externalReference string) ApiGetGatewayDeletionStatusRequest {
	r.externalReference = &externalReference
	return r
}

func (r ApiGetGatewayDeletionStatusRequest) Execute() (*GatewayDeletionStatus, *http.Response, error) {
	return r.ApiService.GetGatewayDeletionStatusExecute(r)
}

/*
GetGatewayDeletionStatus Get deletion status for a reference in the caller scope

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiGetGatewayDeletionStatusRequest
*/
func (a *DefaultAPIService) GetGatewayDeletionStatus(ctx context.Context) ApiGetGatewayDeletionStatusRequest {
	return ApiGetGatewayDeletionStatusRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
// Execute executes the request
//
//	@return GatewayDeletionStatus
func (a *DefaultAPIService) GetGatewayDeletionStatusExecute(r ApiGetGatewayDeletionStatusRequest) (*GatewayDeletionStatus, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *GatewayDeletionStatus
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DefaultAPIService.GetGatewayDeletionStatus")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/hypershell/v1/gateways/deletion"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.externalReference == nil {
		return localVarReturnValue, nil, reportError("externalReference is required and must be specified")
	}
	if strlen(*r.externalReference) < 1 {
		return localVarReturnValue, nil, reportError("externalReference must have at least 1 elements")
	}
	if strlen(*r.externalReference) > 255 {
		return localVarReturnValue, nil, reportError("externalReference must have less than 255 elements")
	}
