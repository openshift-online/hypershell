...
func TestManagedDatabaseReconciler_Handle_RetainsLatestPendingEventWhileBusy(t *testing.T) {
	...
	firstSecret := "hypershell-managed-db-first"
	first := watcher.Event[*pb.ManagedDatabase]{
		ResourceID: id,
		Type:       watcher.EventUpdated,
		Resource:   &pb.ManagedDatabase{ConnectionSecret: &firstSecret},
	}
	...
	r.mu.Unlock()
	if !ok || pending.Resource.GetConnectionSecret() != firstSecret {
		t.Fatalf("pending = %+v, ok=%v; want the first event retained", pending, ok)
	}

	secondSecret := "hypershell-managed-db-second"
	second := watcher.Event[*pb.ManagedDatabase]{
		ResourceID: id,
		Type:       watcher.EventDeleted,
		Resource:   &pb.ManagedDatabase{ConnectionSecret: &secondSecret},
	}
	...
	if pending.Type != watcher.EventDeleted || pending.Resource.GetConnectionSecret() != secondSecret {
