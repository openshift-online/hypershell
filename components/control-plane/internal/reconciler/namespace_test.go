...
// connection. reconcileNamespace does not touch gRPC, so it is exercised
// directly with an explicit live-namespace set.
...
func TestReconcileNamespace_OrphanWithinGraceIsStampedNotDeleted(t *testing.T) {
	...
	updated, _ := client.CoreV1().Namespaces().Get(ctx, "openshell-gw", metav1.GetOptions{})
...
func TestReconcileNamespace_UnmanagedIsUntouched(t *testing.T) {
	...
	updated, _ := client.CoreV1().Namespaces().Get(ctx, "shared", metav1.GetOptions{})
	if _, ok := updated.Annotations[gateway.GCEligibleSinceAnnotation]; ok {
