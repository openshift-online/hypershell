...
| PR-ENV-2 | Continuous deployment lifecycle (opened/reopened/synchronize/closed; unconditional `openshift-up`; per-PR concurrency) | Present | Deploy workflow triggers on open/reopen/synchronize; release workflow on `closed` (merge or close); `make openshift-up` unconditional with `SKIP_SEED`; shared `concurrency: pr-env-<n>` cancel-in-progress | `.github/workflows/pr-environment.yml`, `.github/workflows/pr-environment-release.yml` | W3 ✅ |
