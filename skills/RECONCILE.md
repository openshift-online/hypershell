...
| CI-SEC-3 | GitHub Actions authenticates to AWS with OIDC; OpenShift login refuses forks | Present | One Tests / E2E caller sets `id-token: write`. Deploy/OpenShift skip unless origin. Cluster-login action refuses fork `pull_request` before AssumeRole. | `.github/workflows/tests.yml`, `.github/actions/openshift-cluster-login/action.yml`, hypershell-gitops `ci-secrets.tf` | CI-SEC ✅ |
