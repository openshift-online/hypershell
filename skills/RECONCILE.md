...
**Codebase commit**: `e61bdac` (CP-OBS-RQ-W1 planned)
...
|---|-------------|--------|-----|---------------|------|
| CP-OBS-07f | Export ready reconcile-queue depth as an observable gauge with unit `{item}` | Missing | No queue-depth instrument or queue callback exists | `components/control-plane/internal/otel/metrics.go`, `components/control-plane/internal/watcher/requeue.go` | CP-OBS-RQ-W1 |
| CP-OBS-07g | Export ready-to-worker queue wait as a histogram in seconds | Missing | No queue-wait instrument or ready-time tracking exists | `components/control-plane/internal/otel/metrics.go`, `components/control-plane/internal/watcher/requeue.go` | CP-OBS-RQ-W1 |
| CP-OBS-07h | Use only the bounded `resource.kind` attribute and no resource identifier | Missing | The queue instruments do not exist | `components/control-plane/internal/otel/metrics.go` | CP-OBS-RQ-W1 |
| CP-OBS-07i | Record one wait observation for coalesced work when `Handle` starts | Missing | The queue does not track the first ready time for a pending key | `components/control-plane/internal/watcher/requeue.go` | CP-OBS-RQ-W1 |
| CP-OBS-07j | Exclude scheduled retry backoff from queue depth and queue wait | Missing | The queue has a backoff floor but does not connect it to telemetry | `components/control-plane/internal/watcher/requeue.go` | CP-OBS-RQ-W1 |

**Scoped coverage:** 0 of 5 changed CP-OBS-07 fields are present. This scoped run does not change the full-corpus coverage table.

**Direction checks:**

- Spec to code: The two instruments, bounded label, coalescing rule, and backoff rule are missing.
- Code to spec: The existing queue coalescing and retry rules agree with the new metric boundaries.
...
**Scope:** Changed CP-OBS-07 queue fields only | **Status:** Planned
