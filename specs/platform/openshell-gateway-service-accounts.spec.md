...
This specification defines OpenShellGatewayServiceAccounts for non-interactive access to an OpenShell gateway. Each OpenShellGatewayServiceAccount uses OAuth 2.0 Client Credentials.
...
- **OpenShellGatewayServiceAccount** is a HyperShell resource for one automation identity on one gateway. HyperShell can revoke it independently. It is not a human user or a Kubernetes ServiceAccount.
- **Client credentials** are the client ID and client secret that authenticate the OpenShellGatewayServiceAccount to Keycloak. The client ID is not secret. The client secret is opaque.
...
- **Creator** is the authenticated HyperShell user that creates the OpenShellGatewayServiceAccount. The OpenShellGatewayServiceAccount remains limited by this user's gateway role. A creator cannot transfer it.
...
- **Service-account client** is the confidential OIDC client for one OpenShellGatewayServiceAccount. Keycloak uses it to issue access tokens for one gateway audience.
- **Reconcile** means compare the OpenShellGatewayServiceAccount record with Keycloak and correct differences.
...
User (management OIDC session)
    |
    | POST /api/hypershell/v1/gateways/{gateway_id}/service_accounts
...
OpenShell Gateway
    ...
    v
Authorized service-account request
```

The client secret authenticates the OpenShellGatewayServiceAccount to Keycloak. A client SHALL never send it to the gateway.

The access token is a bearer token. HyperShell SHALL not store it in the OpenShellGatewayServiceAccount record. Later HyperShell responses SHALL not return it.

### Credential and Token Lifetimes

```text
OpenShellGatewayServiceAccount validity period:           1 hour .. 365 days (default 90 days)
...
Disabling or deleting a service-account client prevents new access-token issuance. The gateway validates access tokens locally with the issuer's JWKS. It does not introspect each request.
...
OpenShellGatewayServiceAccount is a first-class, gateway-scoped API resource.
...
The API server SHALL add the OpenShellGatewayServiceAccount table through a forward migration. The migration SHALL add foreign keys and indexes for status and expiration. It SHALL also add a partial unique index for active `(gateway_id, name)` values.
...
Deleting an OpenShellGatewayServiceAccount or Gateway SHALL preserve referential integrity. The deletion SHALL also preserve non-secret audit records.

Operators SHALL remove or reconcile all OpenShellGatewayServiceAccounts before a database rollback. This step prevents the rollback from leaving an enabled service-account client in Keycloak.
...
Credential replacement uses four operations: create a new OpenShellGatewayServiceAccount, update the consumer, revoke the old account, and delete the old account. Revocation is the immediate security action. Deletion removes the identity and visible resource.
...
```json
{
  "id": "35GtY7ExampleOpenShellGatewayServiceAccount",
...
An active duplicate name SHALL return `409 Conflict`. A network retry SHALL NOT create multiple service-account clients with the same gateway and name.

If provisioning fails, HyperShell SHALL first verify whether the service-account client exists. If it does not exist, HyperShell SHALL soft-delete the provisioning reservation. The caller can then retry the name.
...
HyperShell SHALL disable and remove the partial service-account client through reconciliation.
...
gRPC watches and controller events SHALL contain only non-secret OpenShellGatewayServiceAccount metadata. Protobuf resource messages and generic event payloads SHALL never contain a client secret.
...
OpenShellGatewayServiceAccount authorization SHALL evaluate the `gateway_id` in the nested route. The generic viewer rule SHALL NOT authorize these operations.
...
The `platform:admin` role does not grant OpenShell gateway access. A platform administrator needs a gateway binding to create or inspect OpenShellGatewayServiceAccounts.

The API SHALL enforce creation quotas to limit the number of Keycloak clients. The default limit SHALL be 10 active OpenShellGatewayServiceAccounts for each creator on each gateway. The default gateway quota SHALL be 100 active OpenShellGatewayServiceAccounts.

Operators MAY lower either quota. Expired and revoked OpenShellGatewayServiceAccounts do not count toward these quotas.

## Keycloak Representation

### Federated Identity Boundary

The configured Keycloak realm issues access tokens and stores OpenShellGatewayServiceAccount identities. [HYPERSHELL-100](https://redhat.atlassian.net/browse/HYPERSHELL-100) defines one global, public Keycloak deployment. That deployment brokers human login to Red Hat SSO.

HyperShell SHALL create each service-account client and service-account user directly in this realm. It SHALL NOT create a human identity in Red Hat SSO.

Keycloak is authoritative for service-account identities, client credentials, role mappings, and issued access tokens. PostgreSQL is authoritative for the non-secret OpenShellGatewayServiceAccount record and its requested lifecycle state.

[HYPERSHELL-130](https://redhat.atlassian.net/browse/HYPERSHELL-130) defines infrastructure-controller identities as a separate authorization domain. These identities SHALL NOT receive or reuse tenant OpenShellGatewayServiceAccount roles or client credentials.

### One Confidential Client per OpenShellGatewayServiceAccount

HyperShell SHALL create a separate service-account client for each OpenShellGatewayServiceAccount. OpenShellGatewayServiceAccounts SHALL NOT share a client secret. Separate clients permit independent attribution, expiration, role changes, and revocation.

The `clientId` SHALL be `hs-sa-{gateway-id}-{service-account-id}`. The Keycloak display name SHALL contain the user-visible OpenShellGatewayServiceAccount name.
...
Keycloak includes a role only when both the service-account assignment and the service-account client scope permit it. HyperShell SHALL configure both controls:

1. Resolve the selected gateway client `{gateway-name}-{gateway-id}` and its `openshell-user` and `openshell-admin` roles.
2. Add only the required gateway-client roles to the service-account client's role scope mappings while keeping `fullScopeAllowed = false`.
3. Resolve the service-account client's service-account user through the Keycloak Admin API.
...
An access token for an administrator OpenShellGatewayServiceAccount has this security-relevant form:
...
The create operation SHALL provision and validate the service-account client before it returns `201 Created`. HyperShell SHALL test the Client Credentials grant before it sets status `ready`.
...
#### Scenario: Owner creates an administrator OpenShellGatewayServiceAccount

- GIVEN user-a has `gateway:owner` on gateway `gw-alpha`
- AND `gw-alpha` is running with a provisioned Keycloak gateway client
- WHEN user-a creates an OpenShellGatewayServiceAccount named `deploy-bot` with `role=openshell-admin`
...
- GIVEN Keycloak returns an error while a service-account client is being configured
...
HyperShell SHALL provision the service-account client and service-account user in the configured Keycloak realm. This operation SHALL require no manual Keycloak administration.

Keycloak SHALL remain the system of record for service-account identities, client credentials, role mappings, and access-token issuance. HyperShell SHALL store only the non-secret OpenShellGatewayServiceAccount record.

The upstream human identity provider SHALL NOT create or store a service-account user. Client names, attributes, authorization, and audit records SHALL distinguish these identity types:

- Tenant OpenShellGatewayServiceAccounts
- Human identities
- Infrastructure-controller identities

#### Scenario: OpenShellGatewayServiceAccount uses the global Keycloak issuer

- GIVEN production HyperShell uses the global Keycloak issuer with Red Hat SSO as its upstream human identity provider
- WHEN an authorized user creates an OpenShellGatewayServiceAccount
...
The OpenShellGatewayServiceAccount `expires_at` controls how long Keycloak accepts the client credentials. It does not set the access token's `exp`. Callers SHALL NOT set a custom access-token lifetime.

HyperShell SHALL enforce `expires_at` by disabling the service-account client. Version 1 SHALL NOT depend on Keycloak's preview client-secret-expiration capability. The opaque client secret does not expire by itself.

#### Scenario: A 12-month OpenShellGatewayServiceAccount receives a short access token

- GIVEN an OpenShellGatewayServiceAccount expires 365 days after creation
- WHEN its CI job performs a Client Credentials grant while the OpenShellGatewayServiceAccount is ready
...
- AND the CI job MAY request another short-lived access token until the OpenShellGatewayServiceAccount expires or is revoked

#### Scenario: Access token expires during a long job

- GIVEN a CI job still holds valid OpenShellGatewayServiceAccount client credentials
...
Every OpenShellGatewayServiceAccount SHALL target exactly one Gateway. Its audience mapper SHALL add only that gateway's OIDC audience. Its role mapper and scope mappings SHALL reference only that gateway client.
...
- GIVEN Keycloak issued an access token for an OpenShellGatewayServiceAccount on `gw-alpha`
...
- GIVEN Keycloak issued an access token for an OpenShellGatewayServiceAccount
...
- AND the OpenShellGatewayServiceAccount SHALL gain no ability to create gateways, RoleBindings, or other OpenShellGatewayServiceAccounts

#### Scenario: Automation needs two gateways

- GIVEN a CI workflow needs both `gw-alpha` and `gw-beta`
- WHEN the workflow is configured
- THEN an authorized user SHALL create one OpenShellGatewayServiceAccount on each gateway
...
Each active OpenShellGatewayServiceAccount SHALL remain subject to the creator's current gateway role. HyperShell SHALL reconcile the OpenShellGatewayServiceAccount when that role changes.

#### Scenario: Owner deliberately selects the lower role

- GIVEN user-a has `gateway:owner` on `gw-alpha`
- WHEN user-a creates an OpenShellGatewayServiceAccount with `role=openshell-user`
- THEN HyperShell SHALL assign only `openshell-user` to the service-account identity
...
#### Scenario: Viewer creates a user OpenShellGatewayServiceAccount

- GIVEN user-b has only `gateway:viewer` on `gw-alpha`
- WHEN user-b creates an OpenShellGatewayServiceAccount with the default role
...
- AND it SHALL create neither an OpenShellGatewayServiceAccount record nor a Keycloak client

#### Scenario: Creator is downgraded from owner to viewer

- GIVEN user-a created an `openshell-admin` OpenShellGatewayServiceAccount on `gw-alpha`
- WHEN user-a's effective binding on `gw-alpha` changes from owner to viewer
- THEN HyperShell SHALL remove `openshell-admin` from the Keycloak service-account user and client scope
- AND the OpenShellGatewayServiceAccount role SHALL become `openshell-user`
- AND the downgrade SHALL be permanent for that OpenShellGatewayServiceAccount even if user-a is later promoted
- AND an already-issued admin JWT MAY remain usable only until its short `exp`

#### Scenario: Creator loses gateway access

- GIVEN user-a created an OpenShellGatewayServiceAccount on `gw-alpha`
- WHEN user-a has no remaining owner or viewer binding on `gw-alpha`
- THEN HyperShell SHALL permanently revoke that OpenShellGatewayServiceAccount
- AND restoring user-a's binding SHALL NOT re-enable the old client secret

### Requirement: Expiration, Revocation, and Deletion

HyperShell SHALL check expiration at least once per minute. It SHALL disable the service-account client no later than one minute after `expires_at`. It SHALL then mark the OpenShellGatewayServiceAccount `expired`.

An expired, revoked, or deleted service-account client SHALL NOT issue new access tokens. HyperShell SHALL enforce expiration without Keycloak's preview client-secret-expiration feature. A delay or failure SHALL produce an operator alert.

Revocation and deletion are separate operations. Revocation is explicit and permanent. `POST .../{service_account_id}/revoke` SHALL disable the service-account client before it marks the OpenShellGatewayServiceAccount `revoked`.

A revoked OpenShellGatewayServiceAccount retains its non-secret metadata and audit events. A successful revoke response means that Keycloak cannot issue another access token. Repeated revocation SHALL succeed without changing this result. HyperShell SHALL provide no re-enable operation.

Deletion performs final identity cleanup. `DELETE .../{service_account_id}` SHALL revoke a ready OpenShellGatewayServiceAccount first. It SHALL then delete the service-account client and its service-account user. Finally, it SHALL soft-delete the OpenShellGatewayServiceAccount record.

List and get operations SHALL exclude soft-deleted OpenShellGatewayServiceAccounts. A separate audit event SHALL remain after deletion. This specification does not set the audit-retention period.

Deletion of an already revoked OpenShellGatewayServiceAccount SHALL be valid. A `204 No Content` response means that the service-account client is absent. It also means that normal read operations cannot find the OpenShellGatewayServiceAccount.

The API SHALL persist a revoke or delete request before it calls Keycloak. If Keycloak is unavailable, the API SHALL return `202 Accepted`. The response SHALL contain a standard OpenShellGatewayServiceAccount with status `revoking` or `deleting`.
...
The API SHALL return `200` for revoke only after it observes the disabled service-account client. It SHALL return `204` for delete only after it observes that the service-account client is absent.

An access token issued before disablement MAY remain valid until its `exp`.

#### Scenario: OpenShellGatewayServiceAccount expires

- GIVEN a ready OpenShellGatewayServiceAccount reaches `expires_at`
...
#### Scenario: Owner revokes a viewer-created OpenShellGatewayServiceAccount

- GIVEN user-b created an `openshell-user` OpenShellGatewayServiceAccount as a viewer
- AND user-a is an owner of the same gateway
- WHEN user-a revokes that OpenShellGatewayServiceAccount
- THEN HyperShell SHALL disable its Keycloak client and retain non-secret OpenShellGatewayServiceAccount metadata
- AND user-b SHALL not be able to request another access token with the old client secret

#### Scenario: Owner deletes a revoked OpenShellGatewayServiceAccount

- GIVEN an owner previously revoked an OpenShellGatewayServiceAccount
- WHEN the owner deletes that OpenShellGatewayServiceAccount
...
Version 1 SHALL replace an OpenShellGatewayServiceAccount instead of changing its client secret. Use this replacement flow:

1. Create an OpenShellGatewayServiceAccount with the required role and expiration.
2. Install its client credentials in the consumer.
3. Verify that the consumer can request a valid access token.
4. Revoke the old OpenShellGatewayServiceAccount.
5. Delete the old OpenShellGatewayServiceAccount when its metadata is no longer needed.

The old and replacement OpenShellGatewayServiceAccounts MAY remain active at the same time. Both accounts count toward quota. This overlap prevents downtime during the change.
...
- GIVEN `nightly-build-v1` is an active OpenShellGatewayServiceAccount used by CI
...
Gateway deletion SHALL revoke every related OpenShellGatewayServiceAccount. It SHALL also delete the gateway client, console client, and all related service-account clients.
...
#### Scenario: Gateway deletion removes service-account clients

- GIVEN `gw-alpha` has three active OpenShellGatewayServiceAccounts
- WHEN `gw-alpha` is deleted
- THEN HyperShell SHALL disable all three service-account clients before gateway teardown is considered complete
- AND it SHALL delete those clients and their service-account users through Keycloak client deletion
- AND normal read operations SHALL exclude the soft-deleted OpenShellGatewayServiceAccounts
...
- GIVEN a viewer opens OpenShellGatewayServiceAccounts for a gateway
...
1. Create the OpenShellGatewayServiceAccount once through HyperShell.
...
- GIVEN a CI secret store injects the OpenShellGatewayServiceAccount client ID and secret
...
Each service-account client has its own service-account user and stable JWT `sub`. It does not inherit the creator's OpenShell workspace memberships.

An `openshell-admin` OpenShellGatewayServiceAccount can perform OpenShell administrative operations. OpenShell permits this role to bypass workspace membership.
...
#### Scenario: openshell-user OpenShellGatewayServiceAccount has no workspace membership

- GIVEN a viewer creates an `openshell-user` OpenShellGatewayServiceAccount
- AND no OpenShell workspace contains the returned service-account subject
...
The OpenShellGatewayServiceAccount create request SHALL contain no OpenShell scope list. HyperShell configures gateways with an empty `scopes_claim`. This setting disables OpenShell scope enforcement.

Access-token scopes provide no additional control while scope enforcement is disabled. A later scope design SHALL define one gateway-wide contract. Tests SHALL cover human, console, and service-account access tokens together.
...
- Gateway ID and OpenShellGatewayServiceAccount ID
...
HyperShell SHALL disable body capture for OpenShellGatewayServiceAccount create requests and Keycloak token-endpoint requests.

### Requirement: Reconciliation and Drift Repair

The PostgreSQL OpenShellGatewayServiceAccount record SHALL define the required Keycloak state. Reconciliation SHALL restore these settings:
...
- GIVEN an administrator manually changes a service-account client to `fullScopeAllowed=true`
- WHEN HyperShell reconciles that OpenShellGatewayServiceAccount
...
- The distinct workspace-membership requirement for `openshell-user` OpenShellGatewayServiceAccounts.
