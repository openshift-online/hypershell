...
2. **Networking Gateway** — Named `hsgw` in the `openshift-ingress` namespace. Provides a shared HTTPS ingress point for all tenant GRPCRoutes:
   ```yaml
     ...
     - name: grpc
       ...
       allowedRoutes:
         namespaces:
           from: All
   ```
   The `<base-domain>` is read from `ingresses.config.openshift.io/cluster` `.spec.domain` (e.g., `apps-crc.testing`). The `allowedRoutes.namespaces.from: All` permits GRPCRoutes from any tenant namespace. The `hsgw-tls` Secret contains a wildcard certificate for `*.hsgw.<base-domain>` — generated by `setup-gateway-api.sh` on CRC or provisioned by cert-manager in production.
...
1. **GRPCRoute** — In the tenant namespace, referencing the networking Gateway:
   ...
   spec:
     ...
     - name: hsgw
       namespace: openshift-ingress
     hostnames:
     - <gateway-name>-<namespace>.hsgw.<base-domain>
     rules:
     - backendRefs:
       - name: openshell-gateway
         port: 8080
...
GRPCRoute hostnames follow the pattern: `<gateway-name>-<namespace>.hsgw.<base-domain>`

Examples:
- `openshell-gateway-tenant-a.hsgw.apps-crc.testing`
- `openshell-gateway-tenant-b.hsgw.apps.cluster.example.com`

The `hsgw.` subdomain segment distinguishes Gateway API routes from traditional OpenShift Routes (`*.apps-crc.testing`). On CRC, dnsmasq resolves all subdomains of `apps-crc.testing` (including `*.hsgw.apps-crc.testing`) to the CRC VM IP.
...
- Create, update, patch, get, and delete Deployments, Services, ServiceAccounts, Roles, RoleBindings, ConfigMaps, Jobs, NetworkPolicies, GRPCRoutes, and PersistentVolumeClaims in project namespaces
