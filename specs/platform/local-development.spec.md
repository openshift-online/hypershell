...
The stand-in server SHALL serve TLS. `make kind-up` SHALL generate a self-signed CA and a server certificate for `postgres.external-cloud-db.svc.cluster.local` with `openssl` on the first run (stored in a `postgres-tls` Secret in `external-cloud-db` and reused on later runs), and SHALL start PostgreSQL with `ssl=on` using them. After the Deployment is available, `make kind-up` SHALL confirm `SHOW ssl` is `on` and SHALL succeed a `sslmode=verify-full` client connection from the platform namespace using the admin Secret's host, CA, and password. `pg_isready` alone is not sufficient: it succeeds without TLS.

`make kind-up` SHALL create the `hypershell-gateway-database-admin` Secret in the platform namespace with the stand-in server's admin connection: `host`, `port`, `user`, `password`, `dbname`, `sslmode: verify-full` and the generated CA as `sslrootcert`. The controller Deployment mounts it at `/etc/hypershell/gateway-database`; the control plane validates the files at startup and provisions each gateway's database and role over a `verify-full` connection, exactly as in production. After the controller Deployment is applied, `make kind-up` SHALL set `hostAliases` on it so `postgres.external-cloud-db.svc.cluster.local` resolves to the stand-in Service ClusterIP from `/etc/hosts`. That keeps the certificate SAN used by `verify-full` and does not depend on Kind CoreDNS, which CI has seen time out (`lookup ... i/o timeout`) after the in-cluster `*.hypershell.localhost` CoreDNS restart. No credentials namespace is created and no database resource is seeded through the API. `make kind-up` SHALL remove a leftover `hypershell-managed-db-kind` namespace from earlier versions of the environment. See [`openshell-gateway-database.spec.md`](./openshell-gateway-database.spec.md) for the full lifecycle.
...
`make kind-up` SHALL seed all resources needed for a functional local environment: a GatewayRelease and a Gateway with OIDC configuration pointing at the local Keycloak instance. The ManagedCluster is NOT seeded: the Kind overlay runs the control plane with `HYPERSHELL_MANAGED_CLUSTER_NAME=local-kind`, and the control plane registers that record itself (`managed-cluster-registration.spec.md`). Seeding SHALL wait for the `local-kind` ManagedCluster to appear in `GET /managed_clusters` (the control plane registers within its startup backoff) and SHALL use its id for the Gateway; it SHALL NOT `POST /managed_clusters`, since a manually created record would make the control plane's registration a 409 conflict. The seeding step obtains a Bearer token from Keycloak using the admin user (not the control-plane service account), then creates each resource via the REST API. Seeding SHALL be reuse-or-create for the named seed resources (`dev-release`, `dev-gateway`): when a resource with that name already exists, the command SHALL reuse its id and SHALL NOT create another. If any seed step fails, it SHALL warn and continue rather than abort, unless `SEED_STRICT=true`. This makes `kind-up` fully self-contained -- a developer gets a working gateway without any manual API calls after the initial setup.
