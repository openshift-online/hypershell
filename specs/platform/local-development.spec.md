...
`make kind-up` SHALL create the credentials namespace `hypershell-managed-db-kind` containing the `hypershell-managed-db-credentials` Secret with the stand-in server's admin connection (`host`, `port`, `user`, `password`, `dbname`, and `sslmode: disable`). The credentials live in their own reserved-prefix namespace, not in the HyperShell platform namespace, mirroring production where the platform team stages them before HyperShell is installed.

During the resource seeding step, `kind-up` SHALL register the stand-in server as a ManagedDatabase named `openshell-db` with `connection_secret: hypershell-managed-db-kind` via the REST API. The ManagedDatabaseReconciler then verifies connectivity and admin capability, and the GatewayReconciler provisions each gateway's database and role on that server. See [`openshell-gateway-database.spec.md`](./openshell-gateway-database.spec.md) for the full lifecycle.
