...
| Deprecate standalone run/db targets in component Makefiles | `make run`, `make run-no-auth`, `make db/setup`, and `make db/teardown` in `components/api-server/Makefile` are superseded by `make kind-up`. Component Makefiles retain build, test, and codegen targets (`make binary`, `make test`, `make generate`, `make proto`); the root Makefile owns "run the full stack" |
