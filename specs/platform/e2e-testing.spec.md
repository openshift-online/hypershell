...
`checks.yml` SHALL provide a `checks-gate` job (`CI Gate`) that runs with `if: always()`, reads every other job's rolled-up `result` via `needs`, and fails unless `detect-changes` succeeded and no other job failed or was cancelled (a job skipped by path filtering SHALL pass the gate). Because it always runs, it is never left pending by path-filtered skips, so this is one of the two checks to mark required in branch protection (the other being `tests.yml`'s own `CI Gate`). Both gate jobs SHALL be named plain `CI Gate` rather than `Checks CI Gate` / `Tests CI Gate`, since the workflow run each belongs to already disambiguates them in the PR checks list and the branch-protection required-checks picker.
...
- GIVEN branch protection requires `checks.yml`'s `CI Gate`
...
- AND skipped jobs SHALL NOT fail the `CI Gate` check

#### Scenario: Tests Gate For Branch Protection

- GIVEN branch protection requires `tests.yml`'s `CI Gate`
