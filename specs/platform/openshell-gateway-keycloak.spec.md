...
**Machine accounts:** `openshell-gateway-machine-accounts.spec.md` - confidential clients for automation
...
- **MachineAccount lifecycle change** → HyperShell creates, disables, or deletes the related machine client. See `openshell-gateway-machine-accounts.spec.md`.
...
| `directAccessGrantsEnabled` | `true` | Retained for human-client compatibility. New automation SHALL use MachineAccount client credentials instead of a human username and password. |
...
Keycloak does not delete machine clients when it deletes the gateway client. Gateway deletion SHALL disable and delete each related machine client. It SHALL also delete the gateway and console clients. See [`openshell-gateway-machine-accounts.spec.md`](./openshell-gateway-machine-accounts.spec.md).
...
- AND it SHALL separately disable and delete every machine client for that gateway
