...
|---|---|---|
| `admin_users` | string[] | Keycloak usernames assigned `openshell-admin` on the gateway's OIDC client. Required on create (at least one). Used for visibility scoping. See keycloak spec |
