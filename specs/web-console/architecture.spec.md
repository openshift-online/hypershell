...
The API server scopes gateway visibility to the owning user — the gateway list returns only gateways created by the authenticated user (see [`platform/openshell-gateway-keycloak.spec.md`](../platform/openshell-gateway-keycloak.spec.md)). The provisioning action SHALL be available in the primary gateway experience. The gateway provisioning form SHALL NOT collect OIDC configuration — the API server auto-provisions Keycloak OIDC clients and populates OIDC fields when a gateway is created. The form SHALL auto-populate the `admin_users` field with the authenticated user's identity so the creating user is the gateway admin by default.
