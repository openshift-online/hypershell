...
The API server scopes gateway visibility by RBAC RoleBinding membership — the gateway list returns only gateways in fleets where the authenticated user has a RoleBinding (see [`security/rbac-enforcement.spec.md`](../security/rbac-enforcement.spec.md)). The provisioning action SHALL be available in the primary gateway experience. The gateway provisioning form SHALL NOT collect OIDC configuration — the control plane auto-provisions Keycloak OIDC clients and populates OIDC fields when a gateway is created. Gateway provisioning requires an existing fleet; the user's `fleet:owner` or `fleet:editor` RoleBinding on the selected fleet provides automatic admin-tier access to the new gateway via the OIDC Role Bridge (see [`platform/openshell-gateway-keycloak.spec.md`](../platform/openshell-gateway-keycloak.spec.md)).
