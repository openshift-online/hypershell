...
The API server scopes gateway visibility by RBAC RoleBinding membership — the gateway list returns only gateways where the authenticated user has a RoleBinding (see [`platform/openshell-gateway-keycloak.spec.md`](../platform/openshell-gateway-keycloak.spec.md)). The provisioning action SHALL be available in the primary gateway experience. The gateway provisioning form SHALL NOT collect OIDC configuration — the control plane auto-provisions Keycloak OIDC clients and populates OIDC fields when a gateway is created. When a user provisions a gateway, the UI SHOULD auto-create a RoleBinding for the authenticated user with the `admin` role so the creating user has admin access by default.
