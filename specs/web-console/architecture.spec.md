...
The gateway collection and gateway details SHALL display the managed-cluster name for every gateway with a non-empty `cluster_id`. Gateway details SHALL resolve one stable cluster identifier through the managed-cluster API. The gateway collection SHALL normalize and deduplicate the visible page's non-empty cluster identifiers, resolve them through one bounded managed-cluster list request, and cache that batch by its normalized identifier set for 60 seconds. Both experiences SHALL render explicit loading and unavailable states without exposing the raw identifier as a display fallback. Gateways with an empty `cluster_id` SHALL display `Hub cluster` in both experiences.
