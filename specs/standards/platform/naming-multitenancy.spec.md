...
| `hypershell-managed-db-<name>` | Secret | Control Plane Instance | **Reserved prefix.** Administrative connection credentials for a registered external PostgreSQL server (`ManagedDatabase` with `provider: external`). A `ManagedDatabase.connection_secret` reference is a bare name - never `namespace/name` - that MUST carry this prefix and MUST resolve in the control plane instance namespace. The prefix is a security boundary, not a convention: it is what stops an API-level reference from naming an unrelated Secret such as `hypershell-db-app`. No Secret holding anything other than external database admin credentials may take this prefix. See [`openshell-gateway-database-external.spec.md`](../../platform/openshell-gateway-database-external.spec.md). |
