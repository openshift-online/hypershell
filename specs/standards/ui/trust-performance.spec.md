...
Collection views SHALL NOT issue one network request per visible item to resolve related display data. For every supported page size, cold-load request count SHALL remain within a declared bound that does not grow with the number of visible items. Related data SHALL be embedded by the authoritative API, included through a compound response, or resolved through a deduplicated bounded batch lookup. Cache freshness, request deduplication, and warm-cache behavior MAY reduce subsequent traffic but SHALL NOT satisfy the cold-load request bound by themselves.
...
**Verification:** Benchmark cold, warm, throttled, degraded, and high-volume conditions against thresholds declared before final evaluation. Record upstream request count and transferred rows for initial load, page change, search, filter, and sort; test slow typing and a rapid multi-character burst against the declared debounce; and fail an ordinary paginated view that exhausts the collection before first render or an incremental search that requests every intermediate value. Cold-render one row and the maximum supported page with distinct related identifiers; fail when related-data request count grows per row. Verify duplicate identifiers are deduplicated, batch limits are honored, and missing, partial, or failed related-data resolution preserves the primary collection with truthful unavailable states.
