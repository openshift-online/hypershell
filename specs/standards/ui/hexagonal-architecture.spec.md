...
Related resources required to present a collection SHALL be obtained through embedded or included response data or an application-owned batch capability. Infrastructure adapters SHALL normalize and deduplicate batch identifiers, preserve cancellation, enforce the declared batch bound, and reject inconsistent batch responses. A presentation component SHALL NOT invoke a single-resource lookup once per collection row.
...
**Verification:** Search all SDK imports and constructions and confirm they are confined to the adapter/composition surface. Contract-test success, typed errors, abort, one-page collection mapping, literal search values containing quotation, wildcard, and escape metacharacters, search and sort propagation, authoritative page metadata, inconsistent or partial page responses, related-resource batches with distinct and duplicate identifiers, conditional requests, operation polling, credential propagation, and correlation propagation against the generated client. Verify query-language escaping against the authoritative parser or API boundary; an adapter mock that only asserts the serialized expression SHALL NOT establish literal semantics. Fail a list adapter that requests a second page without an explicit application invocation for that page or a collection that performs one related-resource lookup per row.
